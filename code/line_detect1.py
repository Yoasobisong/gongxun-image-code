    ...
    def detect(self, image):
        ...
        if len(approx) == 4:
            ...
            for i in range(4): 
                p1 = approx[i][0]
                p2 = approx[(i + 1) % 4][0]
                distances.append(min(p1[0], cropped_frame.shape[1] - p1[0], p1[1], cropped_frame.shape[0] - p1[1],
                                     p2[0], cropped_frame.shape[1] - p2[0], p2[1], cropped_frame.shape[0] - p2[1]))
            
            closest_edge_index = np.argmin(distances)
            p1 = approx[closest_edge_index][0]
            p2 = approx[(closest_edge_index + 1) % 4][0]
